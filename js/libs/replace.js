    ...
    $.ajax({
      ...
      success: function (response) {
        (request.element = response.replace(/<[?!][\s\w\"-\/:=?]+>/g, "")),
          (request.svgTag = request.element.match(
            /<svg[\w\s\t\n:="\\'\/.#-]+>/g
          ));
        request.svgTagWithoutClass = request.svgTag[0].replace(
          /class=\"[\w\s-_]+\"/,
          ""
        );
        ...
        request.svgTagWithoutClass = request.svgTagWithoutClass.replace(
          ">",
          " " + svg.newClass + ">"
        );

        svg.currency.replaceWith(
          request.element.replace(
            /<svg[\w\s\t\n:="\\'\/.#-]+>/g,
            request.svgTagWithoutClass
          )
        );
      },
    });
